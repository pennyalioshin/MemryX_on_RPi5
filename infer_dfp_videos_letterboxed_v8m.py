...
from yolov8m_letterboxed import yolov8m as YoloModel
...
###################################################################################################
    def __init__(self, video_paths, show=True):
        ...
        self.cap_queue = {i: Queue(maxsize=1) for i in range(self.num_streams)}
        self.dets_queue = {i: Queue(maxsize=1) for i in range(self.num_streams)}
...
###################################################################################################
    def run(self):
        ...
        """
        try: 
            accl = MultiStreamAsyncAccl(dfp='/home/palioshin/BDTI_VLM/yolov8m_on_MXA/yolov8m.dfp')
            accl.set_postprocessing_model('/home/palioshin/BDTI_VLM/yolov8m_on_MXA/yolov8m_post.tflite', model_idx=0)
...
###################################################################################################
    def capture_and_preprocess(self, stream_idx):
        ...
        try: 
            self.cap_queue[stream_idx].put_nowait(frame)
        except Full:
            self.cap_queue[stream_idx].get_nowait()
            self.cap_queue[stream_idx].put_nowait(frame)
...
###################################################################################################
    def postprocess(self, stream_idx, *mxa_output):
        ...
        try: 
            self.dets_queue[stream_idx].put_nowait(dets)
        except Full:
            self.dets_queue[stream_idx].get_nowait()
            self.dets_queue[stream_idx].put_nowait(dets)
